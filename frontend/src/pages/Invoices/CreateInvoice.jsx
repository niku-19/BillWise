    ...
    if (existingInvoice) {
      ...
      const generateNewInvoiceNumber = async () => {
        ...
        try {
          ...
          setFormData(prev => ({ ...prev, invoiceNumber: `INV-${Date.now().toString().slice()}` }))
        }
        ...
        <div className='overflow-x-auto'>
          <table className='w-full divide-y divide-slate-200'>
            <thead className='bg-slate-50'>
              <tr>
                <th className='px-2 sm:px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wide'>Item</th>
                <th className='px-2 sm:px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wide'>Qty</th>
                <th className='px-2 sm:px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wide'>Price</th>
                <th className='px-2 sm:px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wide'>Tax (%)</th>
                <th className='px-2 sm:px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wide'>Total</th>
                <th className='px-2 sm:px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wide'></th>
              </tr>
            </thead>
            <tbody className='bg-white divide-y divide-slate-200'>
              {formData.items.map((item, index) => (
                <tr key={index} className='hover:bg-slate-50'>
                  ...
                  <td>${((item.quantity || 0) * (item.unitPrice || 0) * (1 + (item.taxPercent || 0) / 100)).toFixed(2)}</td>
                  <td className='px-2 sm:px-6 py-4'>
      ...
      <div className='grid grid-cols-1 lg:grid-cols-2 gap-8'>
        ...
        <div className='bg-white p-6 rounded-lg shadow-sm shadow-gray-100 border border-slate-200 flex flex-col justify-center'>
          <div className='space-y-4'>
            ...
            <div className='flex justify-between text-lg font-semibold text-slate-900 border-t-2 border-slate-200 pt-4 mt-4'><p>Total:</p><p>${total.toFixed(2)}</p></div>
          </div>
        </div>
      </div>
    </form>

...
const generateReminderEmail = async (req, res) => {
  ...
  try {
    ...
    const prompt = `
      You are a professional and polite accounting assistant. Write a friendlu reminder email to a client about an overdue or upcoming invoice payment.
      ...
      The tone should be friendly but clear. Keep it consice. Start the email with "Subject:".
    `
    ...
    console.error("Error generating remainder with AI", error);
    res.status(500).json({ message: "Failed to parse invoice data from text", details: error.message })
  }
}
...
const getDashboardSummary = async (req, res) => {
  try {
    ...
    const prompt = `
      ...
      Based on the following summary of their invoice data, provide 2-3 concise nad actionable insights.
      ...
      Example format: {"insight": ["Your revenue is looking string this month", "You have 5 overdue invoices. Consider sending reminders to get paid faster"]}
    `;
